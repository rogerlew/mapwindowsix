        ...
        protected override void OnForward(double[] lp, double[] xy)
        {
            ...
            switch (_mode)
            {
                case Modes.Equitorial:
                    if(cosphi * coslam < -EPS10) throw new ProjectionException(20);
                    xy[Y] = Math.Sin(lp[Phi]);
                    break;
                case Modes.Oblique:
                    double sinphi;
                    if (_sinph0 * (sinphi = Math.Sin(lp[Phi])) + _cosph0 * cosphi * coslam < -EPS10) throw new ProjectionException(20);
                    xy[Y] = _cosph0*sinphi - _sinph0*cosphi*coslam;
                    break;
                default:
                    if(_mode == Modes.NorthPole)coslam = -coslam;
                    if(Math.Abs(lp[Phi] - Phi0) - EPS10 > HalfPi) throw new ProjectionException(20);
                    xy[Y] = cosphi*coslam;
                    break;
            }
