...
namespace MapWindow.Gdal32
{
    ...
    /// GdalIntRaster
    /// </summary>
    public class GdalByteRaster : Raster<byte>
    {
        ...
        public GdalByteRaster(string name, Dataset fromDataset):base(fromDataset.RasterYSize, fromDataset.RasterXSize)
        {
        ...
        public GdalByteRaster(string filename, Dataset fromDataset, Band fromBand)
        ...
        {
            byte[][] result = new byte[sizeY][];
            byte[] rawData = new byte[sizeY *sizeX];

            if (_band == null)
            {
                Raster<byte> ri = Bands[CurrentBand] as Raster<byte>;
                if (ri != null)
                {
            ...
            else
            {
                ...
                {
                    result[row] = new byte[base.NumColumns];
        ...
        public override void WriteRaster(byte[][] buffer, int xOff, int yOff, int xSize, int ySize)
        {
            byte[] rawValues = new byte[xSize * ySize];
