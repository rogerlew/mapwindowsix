...
namespace MapWindow.Components
{
    ...
    public class DirectoryView : ScrollingControl
    {
        ...
        /// Designer variable
        /// </summary>
        //private System.ComponentModel.IContainer components; // the members to be contained
        ...
        protected override void OnInitialize(PaintEventArgs e)
        {
            ...
            System.Drawing.Drawing2D.Matrix oldMat = e.Graphics.Transform;
        ...
        private void AddFiles(ref int tp, int fontHeight)
        {
            ...
            foreach (string file in files)
            {
                ...
                if (fi.ItemType != ItemTypes.Custom)
                {
                    fi.Top = tp;
                    fi.Font = Font;
                    fi.Height = fontHeight;
                    //fi.Navigate += new EventHandler<NavigateEventArgs>(item_Navigate);
                    //fi.SelectChanged += new EventHandler<SelectEventArgs>(item_SelectChanged);
                    _items.Add(fi);
                    tp += fontHeight;
                }
            }
        ...
        void item_SelectChanged(object sender, SelectEventArgs e)
        {
        ...
        {
            if (_directory == null) return;
            string temp = _directory.Trim(System.IO.Path.DirectorySeparatorChar);
            string[] currentDirectoryParts = temp.Split(System.IO.Path.DirectorySeparatorChar);
        ...
        void item_Navigate(object sender, NavigateEventArgs e)
        {
            _directory = e.Path;
            UpdateContent();
            Invalidate();
