...
namespace MapWindow.Components
{
    ...
    public class Legend : ScrollingControl, ILegend
    {
        ...
        public Legend()
        {
            ...
            _editBox.LostFocus += _editBox_LostFocus;
            _indentation = 30;
        ...
        void _editBox_LostFocus(object sender, EventArgs e)
        {
            HideEditBox();
        }
        ...
        public void ClearSelection()
        {
            ...
            if(list.Count > 0)
            {
                list[0].Item.ParentMapFrame().SuspendEvents();
            }
            ...
            if (list.Count > 0)
            {
                list[0].Item.ParentMapFrame().ResumeEvents();
            }
            IsInitialized = false;
            Invalidate();
        }
        ...
        protected virtual void OnIncludeMapFrame(IFrame mapFrame)
        {
            ...
            mapFrame.LayerSelected += Layers_LayerSelected;
        }
        ...
        private Brush HighlightBrush(Rectangle box)
        {
            ...
            if (dark < 0f) dark = 0f;
            Color brtCol = Global.ColorFromHSL(_selectionHighlight.GetHue(), _selectionHighlight.GetSaturation(), bright);
            Color drkCol = Global.ColorFromHSL(_selectionHighlight.GetHue(), _selectionHighlight.GetSaturation(), dark);
        ...
        public Color SelectionHighlight
        {
            ...
            set 
            {
                ...
                _highlightBorderPen = new Pen(Global.ColorFromHSL(_selectionHighlight.GetHue(), _selectionHighlight.GetSaturation(), border));
        ...
        protected virtual PointF OnInitializeItem(DrawLegendItemArgs e)
        {
            ...
            if (topLeft.Y > ControlRectangle.Top - ItemHeight)
            {
                ...
                if (e.Item.LegendSymbolMode == SymbolModes.Symbol)
                {
                    Size s = e.Item.GetLegendSymbolSize();
                    if (s.Height > ih) tempTopLeft.Y += 3;
                }
                if (e.Item.LegendSymbolMode == SymbolModes.Symbol || e.Item.LegendSymbolMode == SymbolModes.GroupSymbol)
                {
                    DrawSymbol(e.Graphics, ref tempTopLeft, itemBox);
                }

                if (e.Item.LegendSymbolMode == SymbolModes.Checkbox)
                {
                ...
                if (itemBox.Item.IsSelected)
                {
                    ...
                    Global.DrawRoundedRectangle(e.Graphics, _highlightBorderPen, itemBox.Textbox);
        ...
        private void DrawPlusMinus(Graphics g, ref PointF topLeft, LegendBox itemBox)
        {
            ILegendItem item = itemBox.Item;
            if (item == null) return;
            if (item.LegendSymbolMode == SymbolModes.Symbol) return; // don't allow symbols to expand
            Point tl = new Point((int)topLeft.X, (int)topLeft.Y);
            tl.Y += (ItemHeight - 8) / 2;
            tl.X += 3;
            ...
            else if (item.LegendItems != null && item.LegendItems.Count() > 0)
            {
