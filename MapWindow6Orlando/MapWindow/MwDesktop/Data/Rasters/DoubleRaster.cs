...
//
// The Initial Developer of this Original Code is Ted Dunsford. Created 2/23/2008 1:06:49 PM
// 
...
namespace MapWindow.Data
{
    ...
    /// DoubleRaster
    /// </summary>
    public class DoubleRaster : Raster, IRaster
    {

        #region Private Variables

        private double _doubleNoDataValue;

        private DoubleRasterList _bands;
        ...
        public DoubleRaster(int numRows, int numColumns)
        {
            base.DataType = typeof(double);
        ...
        protected void Initialize()
        {
            ...
            Value = new DoubleValueGrid(this);
            _doubleNoDataValue = double.MinValue; // Sets it to the appropriate minimum for the int datatype
        }
        ...
        /// </summary>
        /// <returns></returns>
        public new DoubleRaster Copy()
        {
            DoubleRaster copy = MemberwiseClone() as DoubleRaster;
            if (copy == null) return null;
            copy.Bounds = Bounds.Copy();
            copy.Data = new double[NumRows][];
            for (int row = 0; row < NumRows; row++)
            {
                copy.Data[row] = new double[NumColumns];
                for (int col = 0; col < NumColumns; col++)
                {
                    copy.Data[row][col] = Data[row][col];
                }
            }
            copy.Value = new DoubleValueGrid(copy);
            copy.Bands = new DoubleRasterList();
            foreach (DoubleRaster band in _bands)
            {
                if (band != this)
                {
        ...
        public IRaster CopyWindow(string filename, int startRow, int endRow, int startColumn, int endColumn, bool copyValues, bool inRam)
        {
            ...
            DoubleRaster result = new DoubleRaster(numRows, numCols);
        ...
        public IRaster GetWindow(int startRow, int endRow, int startColumn, int endColumn, bool inRam)
        {
            ...
            int numCols = endColumn - startColumn + 1;
            int numRows = endRow - startRow + 1;
            DoubleRaster result = new DoubleRaster();
        ...
        /// <returns>The value of type double of the cell that has a center closest to the specified coordinates</returns>
        public double GetNearestIntValue(double x, double y)
        {
            RcIndex position = this.ProjToCell(x, y);
            if (position.Row < 0 || position.Row >= NumRows) return _doubleNoDataValue;
            if (position.Column < 0 || position.Column >= NumColumns) return _doubleNoDataValue;
        ...
        public double GetNearestIntValue(Coordinate location)
        {
        ...
        public virtual double[] ReadRow(int row)
        {
        ...
        public void SetNearestIntValue(Coordinate location, double value)
        {
            RcIndex position = this.ProjToCell(location.X, location.Y);
            if (position.Row < 0 || position.Row >= NumRows) return;
            if (position.Column < 0 || position.Column >= NumColumns) return;
        ...
        public virtual void WriteValue(int row, int column, double value)
        {
            throw new NotImplementedException(MessageStrings.NotImplemented);
        }
        ...
        public override IList<IRaster> Bands
        {
            ...
            set
            {
                DoubleRasterList tmp = value as DoubleRasterList;
                if (tmp == null && value != null)
                {
                    _bands = new DoubleRasterList();
                    foreach (DoubleRaster s in value)
                    {
                        _bands.Add(s);
                    }
                    return;
        ...
        /// </summary>
        public IList<DoubleRaster> DoubleBands
        {
            get
            {
                return _bands;
        ...
        /// </summary>
        public double DoubleNoDataValue
        {
            get { return _doubleNoDataValue; }
            set { _doubleNoDataValue = value; }
        }

        /// <summary>
        /// Gets or sets the NoDataValue.  This is here for consistency with float, short and int rasters and isn't strictly necessary.
        /// </summary>
