...
//
// The Initial Developer of this Original Code is Ted Dunsford. Created 2/19/2008 10:36:30 AM
// 
...
namespace MapWindow.Data
{
    ...
    /// BinaryShortRaster
    /// </summary>
    internal class BinaryShortRaster : ShortRaster
    {
        #region Constructors
        ...
        public BinaryShortRaster(string filename, int numRows, int numColumns) :
        ...
        public BinaryShortRaster(string filename, int numRows, int numColumns, bool inRam)
        {
                                  ...
                                  bool copyValues, bool inRam)
        {
            ...
            BinaryShortRaster result = new BinaryShortRaster(filename, numCols, numRows, inRam);
        ...
        public override void GetStatistics()
        {
            ...
            for (int row = 0; row < NumRowsInFile; row++)
            {
                for (int col = 0; col < NumColumnsInFile; col++)
                {
                    ...
                    if (val != ShortNoDataValue || val < -30000)
                    {
                        if (val > max) max = val;
                        if (val < min) min = val;
                        double dblVal = val;
        ...
        public new IRaster GetWindow(int startRow, int endRow, int startColumn, int endColumn, bool inRam)
        {
            int numCols = endColumn - startColumn + 1;
            int numRows = endRow - startRow + 1;
            BinaryShortRaster result = new BinaryShortRaster();
        ...
        /// </summary>
        /// <param name="filename">The string filename of the raster to open</param>
        /// <param name="inRam">Boolean, true if the values for the raster data should be loaded into memory</param>
        ...
        /// </summary>
        /// <param name="sourceRaster">The raster of values to paste into this raster.  If the Dx and CellHeight values do not match between the files,
        ...
        public void PasteRaster(IRaster sourceRaster, int startRow, int startColumn)
        {
            ...
            if (sourceRaster.DataType != typeof (short))
            {
                throw new ArgumentException(
                    MessageStrings.ArgumentOfWrongType_S1_S2.Replace("%S1", "sourceRaster").Replace("%S2",
                                                                                                    "BinaryShortRaster"));
            }
            ShortRaster sourceRasterT = (ShortRaster) sourceRaster;
            ...
            if (IsInRam) // ---------------------- RAM BASED ------------------------------------------------------
            {
                ...
                else
                {
                    ...
                    for (int row = 0; row < numPasteRows; row++)
                    {
                        ...
                        for (int col = 0; col < numPasteColumns; col++)
                        {
                            Data[destStartRow + row][destStartColumn + col] = br.ReadInt16();
                        }
                        pm.CurrentValue = row;
            ...
            else // ----------------------------------------- FILE BASED ---------------------------------
            {
                ...
                if (sourceRaster.IsInRam)
                {
                    ...
                    for (int row = 0; row < numPasteColumns; row++)
                    {
                        ...
                        for (int col = 0; col < numPasteColumns; col++)
                        {
                            short val = sourceRasterT.Data[sourceStartRow + row][sourceStartColumn + col];
                            bWriter.Write(val);
                        }
                        pm.CurrentValue = row;
        ...
        public void ReadHeader(string filename)
        {
            ...
            if (dataType != RasterDataTypes.SHORT)
            {
                throw new ArgumentException(
                    MessageStrings.ArgumentOfWrongType_S1_S2.Replace("%S1", filename).Replace("%S2", "BinaryShortRaster"));
            }
            ShortNoDataValue = br.ReadInt16();
        ...
        public override short[] ReadRow(int row)
        {
            ...
            BinaryReader br = new BinaryReader(fs);
            short[] result = new short[NumColumnsInFile];

            for (int col = 0; col < NumColumnsInFile; col++)
            {
                result[col] = br.ReadInt16();
            }
        ...
        /// <summary>
        /// Saves this raster using the current filename.
        /// </summary>
        ...
        public void WriteHeader(string filename)
        {
            ...
            bw.Write(Xllcenter);
            bw.Write(Yllcenter);
            const int val = (int) RasterDataTypes.SHORT;
            bw.Write(val);
            bw.Write(ShortNoDataValue);
        ...
        public override void WriteValue(int row, int column, short value)
        {
        ...
        /// </summary>
        public new virtual int ByteSize
        {
            get { return 2; }
        }
