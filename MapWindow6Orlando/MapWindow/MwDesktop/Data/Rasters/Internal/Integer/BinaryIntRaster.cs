...
namespace MapWindow.Data
{
    ...
    /// </summary>
    internal class BinaryIntRaster : Raster<int>
    {
        #region Constructors
        ...
        public BinaryIntRaster(string filename, int numRows, int numColumns) :
        ...
        public BinaryIntRaster(string filename, int numRows, int numColumns, bool inRam)
        {
                                           ...
                                           bool copyValues, bool inRam)
        {
            ...
            BinaryIntRaster result = new BinaryIntRaster(filename, numCols, numRows, inRam);
        ...
        public new IRaster GetWindow(int startRow, int endRow, int startColumn, int endColumn, bool inRam)
        {
            int numCols = endColumn - startColumn + 1;
            int numRows = endRow - startRow + 1;
            BinaryIntRaster result = new BinaryIntRaster();
            ...
            result.Value = new ValueGrid<int>(result);
            return result;
        }
        ...
        /// <summary>
        /// Prevent the base class method from loading this explicit raster with an internal raster.
        /// </summary>
        ...
        public void PasteRaster(IRaster sourceRaster, int startRow, int startColumn)
        {
            ...
            IntRaster sourceRasterT = (IntRaster) sourceRaster;
            ...
            if (IsInRam) // ---------------------- RAM BASED ------------------------------------------------------
            {
                ...
                else
                {
                    ...
                    for (int row = 0; row < numPasteRows; row++)
                    {
                        ...
                        for (int col = 0; col < numPasteColumns; col++)
                        {
                            Data[destStartRow + row][destStartColumn + col] = br.ReadInt32();
                        }
                        pm.CurrentValue = row;
            ...
            else // ----------------------------------------- FILE BASED ---------------------------------
            {
                ...
                if (sourceRaster.IsInRam)
                {
                    ...
                    for (int row = 0; row < numPasteColumns; row++)
                    {
                        ...
                        for (int col = 0; col < numPasteColumns; col++)
                        {
                            int val = sourceRasterT.Data[sourceStartRow + row][sourceStartColumn + col];
                            bWriter.Write(val);
                        }
                        pm.CurrentValue = row;
        ...
        public void ReadHeader(string filename)
        {
            ...
            if (dataType != RasterDataTypes.INTEGER)
            {
                throw new ArgumentException(
