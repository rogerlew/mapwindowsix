...
//
// The Initial Developer of this Original Code is Ted Dunsford. Created 2/23/2008 12:15:23 PM
// 
...
namespace MapWindow.Data
{
    ...
    /// FloatRaster
    /// </summary>
    public class FloatRaster : Raster, IRaster
    {
       
        #region Private Variables

        private float _floatNoDataValue;

        private FloatRasterList _bands;
        ...
        public FloatRaster(int numRows, int numColumns)
        {
            DataType = typeof(float);
        ...
        /// </summary>
        /// <returns></returns>
        public new FloatRaster Copy()
        {
            FloatRaster copy = MemberwiseClone() as FloatRaster;
            if (copy == null) return null;
            copy.Bounds = Bounds.Copy();
            copy.Data = new float[NumRows][];
            for (int row = 0; row < NumRows; row++)
            {
                copy.Data[row] = new float[NumColumns];
                for (int col = 0; col < NumColumns; col++)
                {
                    copy.Data[row][col] = Data[row][col];
                }
            }
            copy.Value = new FloatValueGrid(copy);
            copy.Bands = new FloatRasterList();
            foreach (FloatRaster band in _bands)
            {
                if (band != this)
                {
        ...
        public override void GetStatistics()
        {
            ...
            for (int row = 0; row < NumRows; row++)
            {
                for (int col = 0; col < NumColumns; col++)
                {
                    ...
                    if (val.CompareTo(_floatNoDataValue) != 0)
                    {
        ...
        /// <summary>
        /// Gets a list of unique float values
        /// </summary>
        /// <returns>A List of floats</returns>
        public List<float> GetUniqueFloatValues()
        {
            ...
            for (int row = 0; row < NumRows; row++)
            {
                ...
                {
                    float val = Data[row][col];
                    if (floatList.Contains(val) == false)
                    {
                        floatList.Add(val);
                    }
                }
        ...
        /// <summary>
        /// Gets a list of unique float values but prevents the addition of values beyond maxCount
        /// </summary>
        /// <param name="maxCount">The maximum count allowed</param>
        /// <param name="overMaxCount">Boolean, true if the number of unique values was greater than maxCount</param>
        /// <returns>A list of floats</returns>
        public List<float> GetUniqueFloatValues(int maxCount, out bool overMaxCount)
        {
            overMaxCount = false;
            List<float> floatList = new List<float>();
            for (int row = 0; row < NumRows; row++)
        ...
        /// <returns>The value of type float of the cell that has a center closest to the specified coordinates</returns>
        public float GetNearestIntValue(double x, double y)
        {
            RcIndex position = this.ProjToCell(x, y);
            if (position.Row < 0 || position.Row >= NumRows) return _floatNoDataValue;
            if (position.Column < 0 || position.Column >= NumColumns) return _floatNoDataValue;
        ...
        public float GetNearestIntValue(Coordinate location)
        {
        ...
        public virtual float[] ReadRow(int row)
        {
        ...
        public void SetNearestIntValue(double x, double y, float value)
        {
            RcIndex position = this.ProjToCell(x, y);
        ...
        public void SetNearestIntValue(Coordinate location, float value)
        {
            RcIndex position = this.ProjToCell(location.X, location.Y);
            if (position.Row < 0 || position.Row >= NumRows) return;
            if (position.Column < 0 || position.Column >= NumColumns) return;
        ...
        public virtual void WriteValue(int row, int column, float value)
        {
            throw new NotImplementedException(MessageStrings.NotImplemented);
        }
        ...
        public override IList<IRaster> Bands
        {
            ...
            set
            {
                FloatRasterList tmp = value as FloatRasterList;
                if (tmp == null && value != null)
                {
                    _bands = new FloatRasterList();
                    foreach (FloatRaster s in value)
                    {
                        _bands.Add(s);
                    }
                    return;
        ...
        /// </summary>
        public IList<FloatRaster> IntBands
        {
            get
            {
                return _bands;
        ...
        /// </summary>
        public float FloatNoDataValue
        {
            get { return _floatNoDataValue; }
            set { _floatNoDataValue = value; }
        }
        ...
        public override double NoDataValue
        {
            get
            {
                return _floatNoDataValue;
            }
            set
            {
                if (value > float.MaxValue)
                {
                    _floatNoDataValue = float.MaxValue;
                }
                else if (value < float.MinValue)
                {
                    _floatNoDataValue = float.MinValue;
                }
                else _floatNoDataValue = Convert.ToSingle(value);
