...
namespace MapWindow.Data
{
    ...
    public class AttributeTable 
    {
        ...
        private void Load()
        {
            ...
            byte[] _byteContent = myReader.ReadBytes(length);
            myReader.Close();
        ...
        public void Write(float number, int length, int decimalCount)
        {
            ...
            //_writer.Write(number);
            // The previous code was replaced by a recommendation from aerosol
            // (wangyq@cams.cma.gov.cn) by Ted Dunsford 3/3/10
            string format = "{0:";
            for (int i = 0; i < decimalCount; i++)
            {
                if (i == 0)
        ...
        private static bool IsNull(byte[] byteCharArray)
        {
            for (int I = 0; I < byteCharArray.Length; I++)
            {
                if (byteCharArray[I] != ' ' && byteCharArray[I] != '\0') return false;
            }
            return true;
        }
