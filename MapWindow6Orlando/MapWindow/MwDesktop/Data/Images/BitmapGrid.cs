    ...
    public class BitmapGrid : ICloneable, IEnumerable<Color>
    {
        ...
        public class BitmapGridEnumerator : IEnumerator<Color>
        {
            ...
            public bool MoveNext()
            {
                ...
                int b = (int)_values[_row * _stride + _column * 4];
                int g = (int)_values[_row * _stride + _column * 4 + 1];
                int r = (int)_values[_row * _stride + _column * 4 + 2];
                int a = (int)_values[_row * _stride + _column * 4 + 3];
        ...
        public Color GetColor(int row, int col)
        {
            ...
            int b = (int)_values[row * Stride + col * 4];
            int g = (int)_values[row * Stride + col * 4 + 1];
            int r = (int)_values[row * Stride + col * 4 + 2];
            int a = (int)_values[row * Stride + col * 4 + 3];
        ...
        {
            return new BitmapGridEnumerator(this) as System.Collections.IEnumerator;
        }
        ...
        private BitmapGrid DoDifference(BitmapGrid compare, bool ignoreAlpha)
        {
            ...
            for (int row = 0; row < maxHeight; row++)
            {
                ...
                {
                    if (row < minHeight && col < minWidth) // Both have values
                    {
                        byte a;
                    ...
                    else if ((row >= h1 || col >= w1) && (row >= h2 || col >= w2)) // neither image has values, but the difference here is zero.
                    {
                        resultValues[row * resStride + col * 4] = (byte)0;
                        resultValues[row * resStride + col * 4 + 1] = (byte)0;
                        resultValues[row * resStride + col * 4 + 2] = (byte)0;
                        resultValues[row * resStride + col * 4 + 3] = (byte)0;
                    }
                    else if (row < h1 && col < w1) // This image has values here but the compare grid doesn't
                    {
                    ...
                    else if (row < h2 && col < w2) // The compare grid has values here but this grid doesn't
                    {
