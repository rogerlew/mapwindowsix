    ...
    public class GradientSlider : Control
    {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (_rightHandle.IsDragging)
            {
                ...
                if (x > Width) x = Width;
                if (x < min) x = min;
                _rightHandle.Position = _min + (x / (float)Width) * (_max - _min);
                if (_leftHandle.Visible)
                {
                    float lw = (float)_leftHandle.Width / (float)Width * (_max - _min);
            ...
            if (_leftHandle.IsDragging)
            {
                ...
                if (x > max) x = max;
                if (x < 0) x = 0;
                _leftHandle.Position = _min + (x / (float)Width) * (_max - _min);
                if (_rightHandle.Visible)
                {
                    float rw = (float)_rightHandle.Width / (float)Width * (_max - _min);
