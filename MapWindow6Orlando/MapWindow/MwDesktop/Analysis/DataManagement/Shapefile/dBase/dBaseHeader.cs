...
namespace MapWindow.Analysis.DataManagement.Shapefile.dBase
{
	...
	public class dBaseHeader
	{
		...
		private FieldDescriptor[] _fieldDescriptions;
		...
		public void AddColumn (string fieldName, char fieldType, int fieldLength, int decimalCount)
		{
			...
			if (_fieldDescriptions == null) _fieldDescriptions = new FieldDescriptor[0];
			int tempLength = 1;  // the length is used for the offset, and there is a * for deleted as the first byte
			FieldDescriptor[] tempFieldDescriptors = new FieldDescriptor[_fieldDescriptions.Length+1];
			...
			tempFieldDescriptors[_fieldDescriptions.Length] = new FieldDescriptor();
		...
		public int RemoveColumn(string fieldName) 
		{
			...
			FieldDescriptor[] tempFieldDescriptors =
				new FieldDescriptor[_fieldDescriptions.Length - 1];
		...
		public void ReadHeader(BinaryReader reader)
		{
			...
			_fieldDescriptions = new FieldDescriptor[_numFields];
			for (int i=0; i<_numFields; i++)
			{
				_fieldDescriptions[i] = new FieldDescriptor();
