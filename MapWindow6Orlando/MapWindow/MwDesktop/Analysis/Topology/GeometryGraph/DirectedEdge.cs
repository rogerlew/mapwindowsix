...
namespace MapWindow.GeometriesGraph
{
    ...
    public class DirectedEdge : EdgeEnd
    {
        ...
        {            
            ...
            if (_isForward)
                Init(inEdge.GetCoordinate(0), inEdge.GetCoordinate(1));            
            else 
            {
                int n = inEdge.NumPoints - 1;
                Init(inEdge.GetCoordinate(n), inEdge.GetCoordinate(n - 1));
            }
        ...
        public virtual void SetDepth(Positions position, int depthVal)
        {
            if (_depth[(int)position] != -999)
                if (_depth[(int)position] != depthVal)
                    throw new TopologyException(MessageStrings.TopologyException_Depth, Coordinate);
