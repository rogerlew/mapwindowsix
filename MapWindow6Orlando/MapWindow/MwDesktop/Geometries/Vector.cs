...
//
// The Original Code is from MapWinGeoProc.Topology
//
    ...
    public class Vector : Coordinate
    {
        ...
        public double Theta
        {
            get
            {
                double mag = Length2D;
                if (mag == 0) return 0;
                double m_Theta = Math.Atan(base.Y/base.X);
                if (X < 0)
        ...
        {
            return new Coordinate(base.X, base.Y, base.Z);
        }
        ...
        {
            // Chris M 2/4/2007
            double length = Math.Sqrt(Math.Pow(base.X, 2) + Math.Pow(base.Y, 2) + Math.Pow(base.Z, 2));
            if (length <= 0)
                return;

            base.X = base.X/length;
            base.Y = base.Y/length;
            base.Z = base.Z/length;
        }
