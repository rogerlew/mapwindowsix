...
using ReadOnlyException=MapWindow.Main.ReadOnlyException;
...
namespace MapWindow.Drawing
{
    ...
    public class FeatureSelection : Changeable, IFeatureSelection
    {
        ...
        private FilterTypes _activeType;
        ...
        /// <exception cref="Main.ReadOnlyException">Occurs if this list is set to read-only in the constructor</exception>
        ...
        public void Clear()
        {
            ...
            foreach (IFeature f in _filter) // modifying the same list we are enumerating causes an exception, even if we aren't deleting
            {
                featureList.Add(f);
            }
            SuspendChanges();
        ...
        public int[] GetCounts(string[] expressions, Tools.ICancelProgressHandler progressHandler, int maxSampleSize)
        {
